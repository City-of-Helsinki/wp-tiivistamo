...
const modal = {
    ...
    init: function () {
        ...
        $('html').on("click", '[data-swiss-modal-action="close"]' ,function(){
            modal.close($(this));
            window.removeEventListener('keypress', modal.setupKeyboardNav, false);
        });

        $('html').on("click", '[data-swiss-modal]' ,function(e){
            e.preventDefault();
            modal.clickOpen($(this));
            window.addEventListener('keypress', modal.setupKeyboardNav, false);
        });
    },

    setupKeyboardNav: function (event) {
        if (event.keyCode === 27) {
            $('[data-swiss-modal-action="close"]').click();
        } else if (event.keyCode === 37) {
            $('[data-swiss-modal-action="prev"]').click();
        } else if (event.keyCode === 39) {
            $('[data-swiss-modal-action="next"]').click();
        }
    },

    // On click open popup and get content for it
    clickOpen: function (clickedButton) {
        // 1. Add data-extra-content to the clickedButton
        // 2. Extend template() function to allow passing extra content
        // 3. Use extra content in template
        ...
        const template = modal.template(content, $(clickedButton).data("swiss-modal"), $(clickedButton).data("swiss-modal-namespace"), $(clickedButton).data("extra-content"));
    ...
    // The popup template
    template: function(content = "<h1>Content has not been set</h1>", style = "default", namespace = "none", extraContent = '') {
        ...
        if(namespace != "none"){
            ...
            if(modal.getNextPrev(-1)){
                ...
                prevUi = `<div class="c-modal__nextprev c-modal__nextprev--prev" data-swiss-modal-namespace="${namespace}"></div>`;
            }
            ...
            if(modal.getNextPrev(1)){
                ...
                nextUi = `<div class="c-modal__nextprev c-modal__nextprev--next" data-swiss-modal-namespace="${namespace}"></div>`;
            }
